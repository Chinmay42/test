...
enum CameraInvigilatorOrientation { landscape, portrait, all }
enum CameraInvigilatorMode { fullscreen, normal }
enum CameraInvigilatorSide { front, back }

class CameraInvigilator extends StatefulWidget {
  final Widget imageMask;
  final CameraInvigilatorMode mode;
  final Widget warning;
  final CameraInvigilatorOrientation orientationEnablePhoto;
  //final Function(File image) onFile;
  final bool enableCameraChange;
  final CameraInvigilatorSide initialCamera;
  final Function(CameraLensDirection direction, List<CameraDescription> cameras)
      onChangeCamera;

  const CameraInvigilator({
    Key key,
    this.imageMask,
    this.mode = CameraInvigilatorMode.normal,
    this.orientationEnablePhoto = CameraInvigilatorOrientation.all,
    //this.onFile,
    this.warning,
    this.onChangeCamera,
    this.initialCamera = CameraInvigilatorSide.back,
    this.enableCameraChange = true,
  }) : super(key: key);
  @override
  _CameraInvigilatorState createState() => _CameraInvigilatorState();
}

class _CameraInvigilatorState extends State<CameraInvigilator> {
  var bloc = BlocCamera();
  var previewH;
  var previewW;
  var screenRatio;
  ...
  void initState() {
    ...
    bloc.cameras.listen((data) {
      ...
      bloc.controllCamera.initialize().then((_) {
        ...
        if (widget.initialCamera == CameraInvigilatorSide.front) bloc.changeCamera();
      });
    });
    ...
    return NativeDeviceOrientationReader(
      ...
      builder: (context) {
        ...
        Widget _getButtonPhoto() {
          if (widget.orientationEnablePhoto == CameraInvigilatorOrientation.all) {
            return _buttonPhoto();
          } else if (widget.orientationEnablePhoto ==
              CameraInvigilatorOrientation.landscape) {
          ...
          body: ConstrainedBox(
            ...
            child: Stack(
              children: <Widget>[
                ...
                if (widget.mode == CameraInvigilatorMode.normal)
                  Padding(
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () async {
            ...
            imageCompressor(croppedFile, showImage,
                "InvigilatorPhoto_${DateTime.now().millisecondsSinceEpoch}");
